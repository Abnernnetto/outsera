...
When('eu abro o carrinho para visualizar meus produtos', async function () {
    await pageFixture.page.getByRole('link', { name: 'Cart', exact: true }).click();
    await expect(pageFixture.page).toHaveURL(/cart\.html/);
});

When('eu inicio a ordem do pedido', async function () {
    await pageFixture.page.getByRole('button', { name: 'Place Order' }).click();
    await expect(pageFixture.page.getByRole('dialog')).toBeVisible();
});

When('eu preencho os meus dados pessoais do formulario para realização da compra', async function () {
  const data = generateOrderData();     
  cartPage = new CartPage(pageFixture.page);

  await cartPage.fillOrderForm(data);
  await pageFixture.page.getByRole('button', { name: 'Purchase' }).click();
});

Then('devo ver a mensagem de sucesso {string} com os dados da compra', async function (mensagemEsperada: string) {
    const p = pageFixture.page;
    const modal = p.locator('.sweet-alert');
  
    await expect(modal).toBeVisible();
    await expect(p.getByRole('heading', { name: new RegExp(mensagemEsperada, 'i') })).toBeVisible();
...
When('eu tento finalizar a compra deixando o campo Nome vazio', async function () {
    const data = generateOrderData();     
    cartPage = new CartPage(pageFixture.page);
  
    await cartPage.fillOrderFormWithoutName(data);
    await cartPage.submitExpectingAlert(/Please fill out Name and Creditcard\.?/i); 
});
...
When('eu tento finalizar a compra deixando o campo Credit Card vazio', async function () {
    const data = generateOrderData();     
    cartPage = new CartPage(pageFixture.page);
...
Then('o carrinho deve ficar vazio sem nenhum produto', async function () {
    const rows = pageFixture.page.locator('#tbodyid tr');
